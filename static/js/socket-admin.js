...
socket.on('pedido-admin-pagamento', function(comanda) {
  ...
  <div class="d-flex justify-content-between align-items-center pedidoCard">
    <div class="d-flex flex-column justify-content-start align-items-start profile w-100">
      ...
      <div>Esperando: <strong class="text-danger" id="tempo-decorrido-pagamente${comanda._id}">00:00:00</strong></div>
    ...
    <div>
      <div class="d-flex flex-column justify-content-center align-items-start">
        <form id="form-finalizar-pagamento${comanda._id}" action="/finalizar_pagamento" method="post">
          ...
          <button type="button" data-comanda="${comanda._id}" class="optionAceitar text-success d-flex justify-content-center align-items-center">
...
var elementosTempoDecorrido = document.querySelectorAll('.tempo-decorrido');

// Para cada elemento encontrado
elementosTempoDecorrido.forEach(function(elemento) {
  // Extrai o ID da comanda do ID do elemento
  var idComanda = elemento.id.replace('tempo-decorrido-pagamente', '');

  // Obtém o timestamp da última atualização da comanda
  var tempoUp = document.getElementById('tempoUp' + idComanda).value;

  // Cria um timer que atualiza o tempo decorrido a cada segundo
  setInterval(function() {
    // Calcula o tempo decorrido em segundos
    var tempoDecorrido = Math.floor((Date.now() - new Date(tempoUp)) / 1000);
    ...
    elemento.textContent = tempoDecorridoFormatado;
